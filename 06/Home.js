...
//简易iscroll
function mTscroll(frame,child,prev,next,speen,fn){
	var idx=0,sw=1;
	if(!fn){fn=function(){};}
	$(frame).stop().animate({scrollTop:0},100);
	...
	function run(type){	
		...
		$(child).slice(0,idx+1).each(function(index, element) {
			w1+=$(this).outerHeight()+parseInt($(this).css("margin-top"))+parseInt($(this).css("margin-bottom"));
		});
		$(child).slice(0,idx).each(function(index, element) {
			w2+=$(this).outerHeight()+parseInt($(this).css("margin-top"))+parseInt($(this).css("margin-bottom"));
		});
		w3=$(child).eq(idx+1).outerHeight()+parseInt($(child).eq(idx+1).css("margin-top"))+parseInt($(child).eq(idx+1).css("margin-bottom"));
		w4=$(child).eq(idx-1).outerHeight()+parseInt($(child).eq(idx-1).css("margin-top"))+parseInt($(child).eq(idx-1).css("margin-bottom"));

		if(type==2){
			if($(frame).scrollTop()+$(frame).height()<w1+w3){
				sw=0;
				$(frame).stop().animate({scrollTop:w1+w3-$(frame).height()},speen,function(){sw=1;});
			}
			if($(frame).scrollTop()>=w2-w4){
				sw=0;
				$(frame).stop().animate({scrollTop:w2-w4},speen,function(){sw=1;});
			}
