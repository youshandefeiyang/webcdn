...
class CursorSparkler{static modes={follow:'follow',trail:'trail'}
...
listen(){window.addEventListener('mousemove',this.onMouseMove)
window.addEventListener('mousedown',this.onMouseDown)
window.addEventListener('mouseup',this.onMouseUp)
document.body.appendChild(this.el)
requestAnimationFrame(this.onAnimationFrame)}
destroy(){this.el.parentElement.removeChild(this.el)
window.removeEventListener('mousemove',this.onMouseMove)
this.shouldAnimate=false}
render(time){if(this.options.disabled){if(this.el.style.display!=='none')this.el.style.display='none'
...
this.el.appendChild(this.sparkles[i].el)}}}
sparkle(startTime){var options=this.options
...
onMouseMove(e){this.x=e.pageX
this.y=e.pageY}
onMouseDown(e){if(!this.originalSparkleFactor){this.originalSparkleFactor=this.options.sparkleFactor}else{this.options.sparkleFactor=this.originalSparkleFactor}
this.options.sparkleFactor*=4}
onMouseUp(e){this.options.sparkleFactor=this.originalSparkleFactor||1
delete this.originalSparkleFactor}
onAnimationFrame(time){if(!this.shouldAnimate)return
if(!this.start)this.start=time
this.render(time)
requestAnimationFrame(this.onAnimationFrame)}}
class Sparkle{static fantasticColors=['yellow','pink','red','orange','purple','cyan']
constructor(options){options=options||{}
...
static getFantasticColor(){return Sparkle.fantasticColors[~~(Sparkle.fantasticColors.length*Math.random())]}
destroy(){this.el.parentElement.removeChild(this.el)}
render(time){var step=normalize(time,this.options.startTime,this.options.startTime+this.options.duration)
...
var sparkler=new CursorSparkler({mode:'trail',})
